# THIS IS A TEMPLATE AND CAN BE IGNORED :)

# CS408 - Assignment #num

Uploaded by [u/NicholasBolen](https://github.com/NicholasBolen/)

You may be able to run the project through the included EXE file, however if there are any issues present with that, you can download the entire `A#num` folder, and open `A#num.pde` in the Processing editor which can be [downloaded here](https://processing.org/download).
...
Window size can be adjusted in the `A#num.pde` file, simply change the `size(x, y, P3D)` function on line #LINENUM (when opened in processing editor) to fit your needs.
