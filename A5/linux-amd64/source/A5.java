...
// Creative Feature - return colour based on current state & colourMode
 public int colorFunc(float density, int x, int y) {
    ...
    // Default
    if (colourMode == 0)
        return color(density, pow(density, 2) * 0.05f, pow(density, 3) * 0.0001f);

    //  Heat map
    if (colourMode == 1) {
        int max = 75;
        // 0  -  25 = black to red
        if (density / max < 0.25f)
            r = 255 * ((density / max) / 0.25f);
        // 25 -  50 = red to green
        else if (density / max < 0.5f) {
            g = 255 * (((density / max) - 0.25f) / 0.25f);
            r = 255 - g;
        }
        // 50 -  99 = green to blue
        else if (density / max < 0.99f) {
            b = 255 * (((density / max) - 0.5f) / 0.49f);
            g = 255 - b;
        }
        // 99 - 100 = blue to white
        else {
            b = 255;
            g = (((density / max) - 0.99f) / 0.01f);
            r = g;
        }
        return color(r, g, b);
    }
    ...
    r = (x / PApplet.parseFloat(width))*255;
    g = 255 - ((x + y) / PApplet.parseFloat(width + height))*255;
    b = 255 - ((x + (height-y)) / PApplet.parseFloat(width + height))*255;
